...
$(document).ready(function() {
  ...
  for (var i = 1; i < maxtarj + 1; i++) {
    ...
    arr_sg_html[i] =
      "<div class=signos id=sg" +
      i +
      "><a><img width=100% heigth=auto ></a></div>";
    arr_bk_html[i] =
      "<div class=signos id=bk" +
      i +
      "><a><img width=100% heigth=auto ></a></div>";
...
function creartablero(grp, cat) {
  ...
  for (var i = 1; i < ntarj + 1; i++) {
    ...
    $("#sg" + p + " img").attr(
      "src",
      "images/img" + imgarrfilen[tdig[p]] + ".png"
    );
    $("#bk" + p + " img").attr(
      "src",
      "images/img" + arrgrupo[grp] + 0 + ".png"
    );
...
function clickimg(n, ntarj) {
  ...
  if (ntoque == 1) {
    ...
    //  SEGUNDO TOQUE
    if (n != nant) {
      // para controlar repeticion del Primer toque
      arrtoque[2] = n;
      //clearTimeout(flgtout);
      mostrar(arrtoque[2]);
      t2 = tact;
      ...
      // se repitio el primer toque  .. es decir n==nant
      ntoque = 1;
    }
  }
  // "SALE DEL TOQUE"
